...
def main():
    ...
    # 测试行：覆盖五分类 + 非枚举类型的边界行
    test_rows = [
        # 1. 复用枚举代码+复用已有标准：字段名与标准"性别代码"同名，码值 = 枚举代码 ENM00002 全集
        {"中文字段名(必填)": "性别代码", "字段所属类型(必填)": "代码枚举类",
         "业务定义(必填)": "标识客户性别的代码", "枚举值(选填)": "1-男;2-女;0-未知"},
        # 2. 复用枚举代码+修改已有标准：码值全集命中（复用枚举代码），字段名语义相关但定义需扩展
        {"中文字段名(必填)": "客户性别代码", "字段所属类型(必填)": "代码枚举类",
         "业务定义(必填)": "零售客户信息中的性别标识", "枚举值(选填)": "1-男;2-女;0-未知"},
        # 3. 修改枚举代码+复用已有标准：码值大量命中但有缺失（需补充）
        {"中文字段名(必填)": "渠道类型代码", "字段所属类型(必填)": "代码枚举类",
         "业务定义(必填)": "办理业务时可通过的渠道类型", "枚举值(选填)": "1-柜面;2-手机银行;3-网上银行;4-智能柜台"},
        # 4. 新增枚举代码新增标准：码值与名称都检索不到
        {"中文字段名(必填)": "量子计算状态代码", "字段所属类型(必填)": "代码枚举类",
         "业务定义(必填)": "量子计算任务执行状态", "枚举值(选填)": "A-量子叠加;B-量子坍缩;C-量子纠缠;D-量子退相干"},
        # 5. 非代码枚举类字段：应被过滤，不处理
        {"中文字段名(必填)": "客户名称", "字段所属类型(必填)": "文本类",
         "业务定义(必填)": "客户的姓名", "枚举值(选填)": ""},
    ]
