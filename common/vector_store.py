...
def create_collection(client, collection_name=None):
    ...
    schema.add_field(
        "name_text", DataType.VARCHAR, max_length=200,
    ...
    schema.add_field(
        "meaning_text", DataType.VARCHAR, max_length=4000,
        enable_analyzer=True, analyzer_params={"type": "chinese"},
    )
    ...
    schema.add_function(Function(
        name="name_bm25", function_type=FunctionType.BM25,
        input_field_names=["name_text"], output_field_names=["name_sparse"],
    ))
    schema.add_function(Function(
        name="meaning_bm25", function_type=FunctionType.BM25,
        input_field_names=["meaning_text"], output_field_names=["meaning_sparse"],
    ))

    index_params = client.prepare_index_params()
    index_params.add_index(
        field_name="name_dense", index_type="HNSW", metric_type="COSINE",
        params={"M": 16, "efConstruction": 200},
    )
    index_params.add_index(
        field_name="meaning_dense", index_type="HNSW", metric_type="COSINE",
        params={"M": 16, "efConstruction": 200},
    )
    index_params.add_index(
        field_name="name_sparse", index_type="SPARSE_INVERTED_INDEX", metric_type="BM25",
    )
    index_params.add_index(
        field_name="meaning_sparse", index_type="SPARSE_INVERTED_INDEX", metric_type="BM25",
    )
...
def search(query_name, query_meaning, top_k=10, collection_name=None, client=None):
    ...
    name_dense_res = client.search(
        collection_name=collection_name, data=[name_vec],
        anns_field="name_dense", limit=sub_limit, output_fields=output_fields,
    )
    meaning_dense_res = client.search(
        collection_name=collection_name, data=[meaning_vec],
        anns_field="meaning_dense", limit=sub_limit, output_fields=output_fields,
    )
    ...
    name_sparse_res = client.search(
        collection_name=collection_name, data=[query_name],
        anns_field="name_sparse", limit=sub_limit, output_fields=output_fields,
    )
    meaning_sparse_res = client.search(
        collection_name=collection_name, data=[query_meaning],
        anns_field="meaning_sparse", limit=sub_limit, output_fields=output_fields,
    )
